"""Run fixed classical baselines on development-only, non-SEPVAL roles."""

from __future__ import annotations

import argparse
import hashlib
import json
import math
from pathlib import Path
import platform

import joblib
import numpy as np
import pandas as pd
import sklearn
from sklearn.impute import SimpleImputer
from sklearn.linear_model import LogisticRegression
from sklearn.pipeline import Pipeline
from sklearn.preprocessing import StandardScaler

from iris_report.iris_sep.workstreams.luna_i_eval_ops.evaluation import (
    apply_calibration,
    fit_intercept_calibration,
    probability_metrics,
    select_tss_threshold,
    threshold_metrics,
)


TARGET = "future_Operational_SEP_label"
META = {"issue_id", "role", "unit_id", "window_begin", "window_end", TARGET}
SEEDS = (7, 13, 26, 42, 73)
IRIS_SEP_ROOT = Path(__file__).resolve().parents[1]
EXPECTED_SOURCE_SHA256 = "ab2bef52a80ebce5c27d2312f031b410843b3fa8e6b351d07a02f3e0ded010ef"
EXPECTED_MANIFEST_SHA256 = "18c10d4fc76a2ce5e03b9a271951003f274435aa00180fcb90e4f2947eedaebb"
EXPECTED_FEATURE_MANIFEST_SHA256 = "7bca82f223f1be0adbd8afc6e30aed238ed52b3bb2339a98fa9c9cbd944436b5"


def sha256_file(path: Path) -> str:
    digest = hashlib.sha256()
    with path.open("rb") as stream:
        for block in iter(lambda: stream.read(1024 * 1024), b""):
            digest.update(block)
    return digest.hexdigest()


def _logit(probability: float) -> float:
    clipped = min(max(probability, 1e-8), 1 - 1e-8)
    return math.log(clipped / (1 - clipped))


def run(source: Path, source_manifest: Path, output_dir: Path) -> dict[str, object]:
    if output_dir.exists():
        raise ValueError("output directory already exists; runs are immutable")
    if sha256_file(source) != EXPECTED_SOURCE_SHA256 or sha256_file(source_manifest) != EXPECTED_MANIFEST_SHA256:
        raise ValueError("source and manifest must match the pinned development-v3 cohort")
    manifest = json.loads(source_manifest.read_text())
    if manifest.get("output_sha256") != EXPECTED_SOURCE_SHA256 or manifest.get("locked_test_rows_present") is not False:
        raise ValueError("source manifest does not bind a non-locked development cohort")
    frame = pd.read_csv(source)
    if not META.issubset(frame.columns) or not frame[TARGET].isin([0, 1]).all():
        raise ValueError("development table contract failure")
    features = [name for name in frame.columns if name not in META]
    if not features or any(name.lower().startswith("future_") for name in features):
        raise ValueError("feature manifest is empty or contains a future outcome")
    feature_manifest_sha256 = hashlib.sha256(json.dumps(features, separators=(",", ":")).encode()).hexdigest()
    if feature_manifest_sha256 != EXPECTED_FEATURE_MANIFEST_SHA256:
        raise ValueError("feature order does not match the pinned allowlist")
    if frame["issue_id"].duplicated().any() or frame.duplicated(["window_begin", "window_end"]).any():
        raise ValueError("duplicate development identity")
    if frame.groupby("unit_id")["role"].nunique().max() != 1:
        raise ValueError("cohort unit spans multiple roles")
    roles = {name: frame.loc[frame["role"] == name].copy() for name in frame["role"].unique()}
    required_roles = {"train", "validation_monitor", "validation_calibration", "validation_threshold"}
    if set(roles) != required_roles or any(group[TARGET].nunique() != 2 for group in roles.values()):
        raise ValueError("every required development role must contain both classes")
    ordered_roles = ("train", "validation_monitor", "validation_calibration", "validation_threshold")
    for left, right in zip(ordered_roles, ordered_roles[1:]):
        left_end = pd.to_datetime(roles[left]["window_end"], utc=True).max()
        right_start = pd.to_datetime(roles[right]["window_end"], utc=True).min()
        if right_start <= left_end + pd.Timedelta(hours=24):
            raise ValueError(f"inclusive 24-hour purge failure between {left} and {right}")
    output_dir.mkdir(parents=True)
    predictions: list[pd.DataFrame] = []
    runs: list[dict[str, object]] = []
    train = roles["train"]
    train_prevalence = float(train[TARGET].mean())
    for seed in SEEDS:
        pipeline = Pipeline([
            ("imputer", SimpleImputer(strategy="median", add_indicator=True, keep_empty_features=True)),
            ("scaler", StandardScaler()),
            ("model", LogisticRegression(
                penalty="elasticnet", solver="saga", l1_ratio=0.5, C=1.0,
                max_iter=10000, class_weight=None, random_state=seed, n_jobs=1,
            )),
        ])
        pipeline.fit(train[features], train[TARGET])
        converged = bool(np.all(pipeline.named_steps["model"].n_iter_ < pipeline.named_steps["model"].max_iter))
        if not converged:
            raise ValueError(f"elastic-net seed {seed} did not converge")
        model_path = output_dir / f"elastic_net_seed_{seed}.joblib"
        joblib.dump(pipeline, model_path)
        for model_name in ("climatology", "elastic_net"):
            logits_by_role: dict[str, np.ndarray] = {}
            for role, group in roles.items():
                if model_name == "elastic_net":
                    logits_by_role[role] = pipeline.decision_function(group[features])
                else:
                    logits_by_role[role] = np.full(len(group), _logit(train_prevalence))
            calibration = fit_intercept_calibration(
                logits_by_role["validation_calibration"],
                roles["validation_calibration"][TARGET],
                role="validation_calibration",
            )
            probability_by_role = {
                role: apply_calibration(logits, calibration) for role, logits in logits_by_role.items()
            }
            threshold = select_tss_threshold(
                roles["validation_threshold"][TARGET],
                probability_by_role["validation_threshold"],
                role="validation_threshold",
            )
            role_metrics = {}
            for role, group in roles.items():
                probability = probability_by_role[role]
                role_metrics[role] = {
                    **threshold_metrics(group[TARGET], probability, threshold.threshold),
                    **probability_metrics(group[TARGET], probability, reference_probability=train_prevalence),
                }
                predictions.append(pd.DataFrame({
                    "issue_id": group["issue_id"], "role": role, "unit_id": group["unit_id"],
                    "label": group[TARGET].astype(int), "model": model_name, "seed": seed,
                    "calibrated_probability": probability,
                }))
            runs.append({
                "model": model_name,
                "seed": seed,
                "calibration_id": calibration.calibration_id,
                "calibration_intercept": calibration.intercept,
                "threshold_id": threshold.threshold_id,
                "threshold": threshold.threshold,
                "metrics": role_metrics,
                "model_artifact": str(model_path.name) if model_name == "elastic_net" else None,
                "model_artifact_sha256": sha256_file(model_path) if model_name == "elastic_net" else None,
                "converged": converged if model_name == "elastic_net" else None,
                "n_iter": int(np.max(pipeline.named_steps["model"].n_iter_)) if model_name == "elastic_net" else None,
            })
    prediction_frame = pd.concat(predictions, ignore_index=True)
    prediction_path = output_dir / "development_predictions.csv"
    prediction_frame.to_csv(prediction_path, index=False, float_format="%.17g")
    # The CSV is the auditable prediction artifact. Recompute every metric from
    # its reloaded values so a receipt can never disagree at a threshold edge.
    reloaded = pd.read_csv(prediction_path, float_precision="round_trip")
    for result in runs:
        for role, group in roles.items():
            saved = reloaded.loc[
                (reloaded["model"] == result["model"])
                & (reloaded["seed"] == result["seed"])
                & (reloaded["role"] == role)
            ]
            if len(saved) != len(group) or saved["issue_id"].duplicated().any():
                raise ValueError("saved prediction coverage failure")
            result["metrics"][role] = {
                **threshold_metrics(saved["label"], saved["calibrated_probability"], result["threshold"]),
                **probability_metrics(saved["label"], saved["calibrated_probability"], reference_probability=train_prevalence),
            }
    receipt = {
        "status": "PASS_DEVELOPMENT_ONLY",
        "source_sha256": sha256_file(source),
        "source_manifest_sha256": sha256_file(source_manifest),
        "feature_manifest_sha256": feature_manifest_sha256,
        "feature_count": len(features),
        "target": TARGET,
        "target_semantics": "PUBLISHER_LEGACY_WINDOW_LABEL_NOT_FINAL_NEW_CROSSING_TARGET",
        "seeds": list(SEEDS),
        "training_prevalence": train_prevalence,
        "role_prevalence": {role: float(group[TARGET].mean()) for role, group in roles.items()},
        "predictions_sha256": sha256_file(prediction_path),
        "runs": runs,
        "environment": {
            "python": platform.python_version(), "numpy": np.__version__,
            "pandas": pd.__version__, "sklearn": sklearn.__version__,
        },
        "locked_test_accessed": False,
        "selection_side_metrics": ["validation_monitor", "validation_calibration", "validation_threshold"],
        "headline_eligible_roles": [],
        "source_hashes": {
            "runner": sha256_file(Path(__file__)),
            "evaluation": sha256_file(IRIS_SEP_ROOT / "workstreams" / "luna_i_eval_ops" / "evaluation.py"),
            "benchmark_contract": sha256_file(IRIS_SEP_ROOT / "config" / "benchmark_contract_v2.json"),
            "evaluation_policy": sha256_file(IRIS_SEP_ROOT / "config" / "evaluation_policy_v1.json")
        },
        "claims_forbidden": ["SEPVAL_SCORE", "FINAL_NEW_CROSSING_SCORE", "BREAKTHROUGH", "OPERATIONAL_CERTIFICATION"],
    }
    receipt_path = output_dir / "receipt.json"
    receipt_path.write_text(json.dumps(receipt, sort_keys=True, indent=2) + "\n", encoding="utf-8")
    return receipt


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--source", type=Path, required=True)
    parser.add_argument("--source-manifest", type=Path, required=True)
    parser.add_argument("--output-dir", type=Path, required=True)
    args = parser.parse_args()
    receipt = run(args.source, args.source_manifest, args.output_dir)
    print(json.dumps({key: receipt[key] for key in ("status", "feature_count", "seeds", "locked_test_accessed")}, sort_keys=True))


if __name__ == "__main__":
    main()
