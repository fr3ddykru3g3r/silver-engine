...
def run(mode: str, output_dir: Path, *, max_epochs: int = 150, patience: int = 20, resume: bool = False, interrupt_after: int | None = None) -> dict[str, Any]:
    ...
    run_config = {"version":"v3","mode": mode, "source_sha256": SOURCE_SHA256, "source_manifest_sha256": MANIFEST_SHA256, "seeds": list(SEEDS), "max_epochs": max_epochs, "patience": patience, "batch_size":128, "shuffle":True, "optimizer":"Adam(lr=1e-4,weight_decay=1e-5)", "scheduler":"ReduceLROnPlateau(factor=0.1,patience=5)"}
