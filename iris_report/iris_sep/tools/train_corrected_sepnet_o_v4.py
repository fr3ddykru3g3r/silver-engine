...
def run(mode: str, output_dir: Path, *, max_epochs: int = 150, patience: int = 20, resume: bool = False, interrupt_after: int | None = None) -> dict[str, Any]:
    ...
    receipt.update({"artifact_version":"v4","partition_dependency":"fixed v6 roles may derive from operational labels; weights are invariant only within those partitions","cuda_verification":"not_run_cpu_only","target_deviation":"scalar log1p(future_SEP_MaxFlux), public SEPNET-O target/config equivalence not established"})
