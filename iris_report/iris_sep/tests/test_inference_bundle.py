...
def fixture():
    runtime = OperatorRuntimePolicy(
        'fixture-policy', 'fixture-calibration', 'b' * 64,
    ...
    receipt = {
        ...
        'bindings': {
            ...
            'operating_thresholds': dict(runtime.operating_thresholds),
            'maximum_age_minutes': dict(runtime.maximum_age_minutes),
            'critical_modalities': list(runtime.critical_modalities),
        },
    }
    ...
    fresh = {
        ...
        for m in runtime.maximum_age_minutes
    }
    return dict(
        admission_policy=admission,
        runtime_policy=runtime,
        ...
        issued_at=datetime(2026, 1, 1, 12, tzinfo=timezone.utc),
        input_schema_sha256=runtime.schema_sha256,
        data_freshness=fresh,
        missing_modalities=[],
        uncertainty={'between_seed_spread': .03, 'calibration_uncertainty': .04, 'input_quality': 1.0},
        model_version='fixture-model',
    )
...
class BundleTests(unittest.TestCase):
    def test_valid_bundle_replays_without_external_policy_or_arrays(self):
        ...
        result = replay_inference_bundle(bundle_bytes=bundle, expected_bundle_sha256=digest)
        self.assertEqual(result['forecast_status'], 'VALID')
        self.assertEqual(result['inference_bundle_sha256'], digest)

    def test_outer_tamper_is_rejected(self):
        bundle, digest = build_inference_bundle(**fixture())
        with self.assertRaises(InferenceBundleError):
            replay_inference_bundle(bundle_bytes=bundle + b' ', expected_bundle_sha256=digest)

    def test_inner_array_tamper_is_rejected_even_with_new_outer_anchor(self):
        bundle, _ = build_inference_bundle(**fixture())
        env = json.loads(bundle)
        env['payload']['arrays']['model_outputs']['bytes_b64'] = 'AAAAAAAAAAA='
        mutated, anchor = reanchor(env)
        with self.assertRaises(InferenceBundleError):
            replay_inference_bundle(bundle_bytes=mutated, expected_bundle_sha256=anchor)

    def test_revision_snapshot_cannot_disagree_with_freshness(self):
        bundle, _ = build_inference_bundle(**fixture())
        env = json.loads(bundle)
        env['payload']['source_revisions']['magnetic'] = 'other'
        mutated, anchor = reanchor(env)
        with self.assertRaises(InferenceBundleError):
            replay_inference_bundle(bundle_bytes=mutated, expected_bundle_sha256=anchor)
    ...
    def test_policy_or_threshold_mutation_breaks_embedded_receipt_binding(self):
        bundle, _ = build_inference_bundle(**fixture())
        env = json.loads(bundle)
        env['payload']['runtime_policy']['operating_thresholds']['MONITOR'] = .25
        env['payload']['threshold']['operating_thresholds']['MONITOR'] = .25
        mutated, anchor = reanchor(env)
        with self.assertRaises(InferenceBundleError):
            replay_inference_bundle(bundle_bytes=mutated, expected_bundle_sha256=anchor)
    ...
    def test_calibration_mutation_is_rejected_by_receipt_static_binding(self):
        ...
        bundle, _ = build_inference_bundle(**fixture())
        env = json.loads(bundle)
        env['payload']['calibration']['intercept'] = 0.15
        aggregated = 0.1
        env['payload']['derived']['calibrated_probability'] = 1 / (1 + math.exp(-(aggregated + 0.15)))
        mutated, anchor = reanchor(env)
        with self.assertRaises(InferenceBundleError):
            replay_inference_bundle(bundle_bytes=mutated, expected_bundle_sha256=anchor)

    def test_admission_policy_mutation_breaks_embedded_receipt_binding(self):
        bundle, _ = build_inference_bundle(**fixture())
        env = json.loads(bundle)
        env['payload']['admission_policy']['supported_through'] = '2028-01-01T00:00:00+00:00'
        mutated, anchor = reanchor(env)
        with self.assertRaises(InferenceBundleError):
            replay_inference_bundle(bundle_bytes=mutated, expected_bundle_sha256=anchor)

    def test_model_or_schema_mutation_breaks_embedded_receipt_binding(self):
        for field, value in [('model_version', 'other-model'), ('input_schema_sha256', 'c' * 64)]:
            with self.subTest(field=field):
                bundle, _ = build_inference_bundle(**fixture())
                env = json.loads(bundle)
                env['payload']['request'][field] = value
                mutated, anchor = reanchor(env)
                with self.assertRaises(InferenceBundleError):
                    replay_inference_bundle(bundle_bytes=mutated, expected_bundle_sha256=anchor)

    def test_evidence_receipt_mutation_is_rejected(self):
        bundle, _ = build_inference_bundle(**fixture())
        env = json.loads(bundle)
        env['payload']['evidence_receipt']['bytes_b64'] += 'A'
        mutated, anchor = reanchor(env)
        with self.assertRaises(InferenceBundleError):
            replay_inference_bundle(bundle_bytes=mutated, expected_bundle_sha256=anchor)

    def test_derived_probability_mutation_is_rejected(self):
        bundle, _ = build_inference_bundle(**fixture())
        env = json.loads(bundle)
        env['payload']['derived']['calibrated_probability'] = 0.99
        mutated, anchor = reanchor(env)
        with self.assertRaises(InferenceBundleError):
            replay_inference_bundle(bundle_bytes=mutated, expected_bundle_sha256=anchor)
    ...
    def test_builder_rejects_unbound_evidence_receipt(self):
        args = fixture()
        receipt = json.loads(args['evidence_bytes'])
        receipt.pop('inference_binding_sha256')
        evidence = json.dumps(receipt, sort_keys=True).encode()
        args['evidence_bytes'] = evidence
        args['expected_evidence_sha256'] = hashlib.sha256(evidence).hexdigest()
        with self.assertRaises(InferenceBundleError):
            build_inference_bundle(**args)

    def test_builder_rejects_nonfinite_arrays(self):
        args = fixture()
        args['model_outputs'] = [float('nan')]
        with self.assertRaises(InferenceBundleError):
            build_inference_bundle(**args)

    def test_builder_rejects_empty_arrays(self):
        for field in ('transformed_features', 'model_outputs'):
            with self.subTest(field=field):
                args = fixture()
                args[field] = []
                with self.assertRaises(InferenceBundleError):
