...
class CorrectedSEPNetOV3Tests(unittest.TestCase):
    ...
    def test_interrupted_resume_is_bitwise_equivalent(self) -> None:
        with TemporaryDirectory() as temp:
            ...
            resumed_state = torch.load(interrupted / "seed_7" / "last.pt", map_location="cpu", weights_only=False)
            uninterrupted_state = torch.load(complete / "seed_7" / "last.pt", map_location="cpu", weights_only=False)
            for key in ("epoch", "stale", "best_loss", "best_epoch", "scheduler"):
                self.assertEqual(resumed_state[key], uninterrupted_state[key])
            for key in resumed_state["model"]:
                self.assertTrue(torch.equal(resumed_state["model"][key], uninterrupted_state["model"][key]))
            for parameter, values in resumed_state["optimizer"]["state"].items():
                for key, value in values.items():
                    self.assertTrue(torch.equal(value, uninterrupted_state["optimizer"]["state"][parameter][key]))
            self.assertTrue(torch.equal(resumed_state["rng"]["torch"], uninterrupted_state["rng"]["torch"]))
