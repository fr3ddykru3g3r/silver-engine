...
def checkpoint_state(model, ema, args, step: int, *, device: torch.device,
                    effective_epochs: int, steps_per_epoch: int):
    """Build a self-describing checkpoint for resumable train-only audits."""
    return {
        ...
        'training_objective': 'denoising plus optional train-only generic/HJ/PIL losses',
...
def main():
    ...
    ap.add_argument('--lr-schedule', choices=['constant', 'cosine'], default='constant',
                    help='Optional train-only schedule; constant preserves the original v2 budget.')
    ap.add_argument('--lambda-generic', type=float, default=0.0,
                    help='Optional train-only generic descriptor loss for generator calibration.')
    ...
    ap.add_argument('--ema-warmup-steps', type=int, default=0,
                    help='Copy model weights into EMA during warmup to avoid an initial random-weight lag.')
    ap.add_argument('--checkpoint-every', type=int, default=0,
                    help='Save numbered checkpoints at this interval; 0 saves only the final checkpoint.')
    args = ap.parse_args()
    ...
    if args.ema_warmup_steps < 0 or args.checkpoint_every < 0:
        raise ValueError('EMA warmup and checkpoint interval must be non-negative')
    ...
    planned_steps = args.max_steps if args.max_steps > 0 else effective_epochs * steps_per_epoch
    scheduler = None
    if args.lr_schedule == 'cosine':
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            opt, T_max=max(1, planned_steps), eta_min=args.lr * 0.05
        )
    ...
    for epoch in range(1, effective_epochs+1):
        for batch in mixed_loader:
            ...
            lhj = denoise * 0.0
            lpil = denoise * 0.0
            # A generic calibration term, when requested, uses the same positive
            # train-only physics batch as the structure terms.  No validation or
            # outer-evaluation rows can enter this path.
            if args.condition != 'base' or args.lambda_generic > 0:
                ...
                if args.lambda_generic > 0:
                    lgeneric = generic_distribution_loss(pfake, praw)
            ...
            total = denoise + ramp * (
                args.lambda_generic*lgeneric + args.lambda_hj*lhj + args.lambda_pil*lpil
            )
            ...
            rec = {
                ...
                'physics_batch_size': args.physics_batch_size if (args.condition != 'base' or args.lambda_generic > 0) else 0,
                'learning_rate': float(opt.param_groups[0]['lr']),
                'ema_warmup_active': bool(args.ema_warmup_steps and step <= args.ema_warmup_steps),
            }
            history.append(rec)
            if step % 20 == 0:
            ...
            if args.checkpoint_every and step % args.checkpoint_every == 0:
                torch.save(
                    checkpoint_state(model, ema, args, step, device=device,
                                     effective_epochs=effective_epochs,
                                     steps_per_epoch=steps_per_epoch),
    ...
    ck = checkpoint_state(model, ema, args, step, device=device,
                          effective_epochs=effective_epochs,
                          steps_per_epoch=steps_per_epoch)
