...
def main():
    ...
    receipt=base/'data/derived/tai_repair_audit.json'
    ...
    for fd in sorted(root.glob('fold_*')):
        ...
        # Preserve the upstream exact-time provenance receipt. The nested
        # manifest is a row-restricted view of an already repaired source; the
        # receipt is intentionally provenance, not a claim that its hash equals
        # the base manifest hash.
        shutil.copy2(receipt,dest/'tai_repair_audit.json')
