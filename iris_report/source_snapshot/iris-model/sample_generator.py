...
def main():
    ...
    # v3 checkpoints intentionally contain optimizer and RNG metadata; load the
    # trusted local artifact explicitly with the full pickle payload.
    ck=torch.load(args.checkpoint,map_location='cpu',weights_only=False); device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')
