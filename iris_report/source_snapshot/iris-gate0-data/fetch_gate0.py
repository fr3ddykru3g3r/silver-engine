...
def parse_harp_map(path: Path):
    ...
    for line in path.read_text(errors='ignore').splitlines():
        ...
        harp=nums[0]; noaas=[n for n in nums[1:] if n>=10000]
...
def fetch_srs(d):
    url=f'{SRS_ROOT}/{d:%Y}/{d:%m}/{d:%Y%m%d}SRS.txt'; p=RAW/'srs'/f'{d:%Y%m%d}SRS.txt'
    try: return d,get(url,p,timeout=40),parse_srs_regions(p.read_text(errors='ignore')),None
    except Exception as ex: return d,None,[],str(ex)


def main():
    ledger=[]; flare_raw=RAW/'goes_mission.csv'; harp_raw=RAW/'all_harps_with_noaa_ars.txt'
    ledger.append(get(NCEI_MISSION, flare_raw))
    try: ledger.append(get(HARP_MAP, harp_raw))
    except Exception as e:
        print('HARP map HTTP fetch failed, retrying HTTPS:', e); ledger.append(get(HARP_MAP.replace('http://','https://'), harp_raw))

    f=pd.read_csv(flare_raw, low_memory=False); cols={c.lower():c for c in f.columns}
    def pick(*names):
        for n in names:
    ...
    sub=f[is_mx & f[cstart].ge(s) & f[cstart].lt(e)].copy(); sub.to_csv(DER/'goes_m1plus_interval.csv',index=False)

    hm=parse_harp_map(harp_raw); hm.to_csv(DER/'harp_noaa_mapping.csv',index=False)
    long=[]
    ...
    longdf=pd.DataFrame(long); longdf.to_csv(DER/'harp_noaa_long.csv',index=False)

    days=[]; d=START
    while d<=END: days.append(d); d+=timedelta(days=1)
    srs_rows=[]; missing=[]
    with ThreadPoolExecutor(max_workers=24) as ex:
        futs={ex.submit(fetch_srs,d):d for d in days}
        for n,fut in enumerate(as_completed(futs),1):
            d,rec,regions,err=fut.result()
            if err: missing.append({'date':d.isoformat(),'url':f'{SRS_ROOT}/{d:%Y}/{d:%m}/{d:%Y%m%d}SRS.txt','error':err})
            else:
                ledger.append(rec)
                for reported,loc in regions:
                    ...
                    srs_rows.append({'report_date':d.isoformat(),'reported_region':reported,'location':loc,'canonical_noaa_candidates':';'.join(map(str,canon)),'resolution':'UNIQUE' if len(canon)==1 else ('AMBIGUOUS' if len(canon)>1 else 'UNMAPPED')})
            if n%100==0: print(f'SRS {n}/{len(days)}',flush=True)
    srs=pd.DataFrame(srs_rows).sort_values(['report_date','reported_region']); srs.to_csv(DER/'srs_region_census.csv',index=False)
    ...
    active=longdf[longdf.noaa_ar.isin(unique_noaa)].copy().sort_values(['harpnum','noaa_ar']); active.to_csv(DER/'active_harps.csv',index=False)

    ars=pd.to_numeric(sub[car],errors='coerce').dropna().astype(int)
    stats={'interval_start':START.isoformat(),'interval_end':END.isoformat(),'m1plus_events':int(len(sub)),'m1plus_events_with_ar':int(len(ars)),'unique_reported_active_regions':int(ars.nunique()),'srs_reports_expected':len(days),'srs_reports_missing':len(missing),'srs_unique_canonical_noaa_regions':len(unique_noaa),'active_harps_from_srs':int(active.harpnum.nunique()),'flare_columns':list(f.columns),'flare_start_col':cstart,'flare_class_col':cclass,'flare_ar_col':car}
