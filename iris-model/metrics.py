...
def threshold_metrics(y, p, threshold: float = 0.5):
    ...
    return {'tp': tp, 'tn': tn, 'fp': fp, 'fn': fn, 'recall': float(tpr), 'fpr': float(fpr),
...
def brier_skill_score(y, p):
    y=np.asarray(y,dtype=float);p=np.asarray(p,dtype=float)
    bs=float(np.mean((y-p)**2))
    climat=float(np.mean((y-y.mean())**2))
    return float(1-bs/climat) if climat>0 else np.nan
...
def region_bootstrap(frame: pd.DataFrame, n_boot: int = 1000, seed: int = 2026, threshold: float = 0.5):
    ...
    keys = ['tss', 'hss', 'auroc', 'brier', 'bss', 'ece10']
