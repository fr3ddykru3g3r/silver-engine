...
class AugmentedDataset(Dataset):
    def __init__(self,real,synthetic_manifest=None,duplicate_count=0,seed=2026):
        ...
        if synthetic_manifest:
            p=Path(synthetic_manifest);self.syn=pd.read_csv(p);self.syn_root=p.parent
            self.items += [('syn',i) for i in range(len(self.syn))]
...
class FocalBCE(nn.Module):
    ...
    def forward(self,logits,target):
        b=nn.functional.binary_cross_entropy_with_logits(logits,target,pos_weight=self.pos_weight,reduction='none');p=torch.sigmoid(logits);pt=torch.where(target>.5,p,1-p)
        return (((1-pt).clamp_min(1e-6)**self.gamma)*b).mean()
...
def main():
    ...
    ap.add_argument('--batch-size',type=int,default=32);ap.add_argument('--steps',type=int,default=1200);ap.add_argument('--eval-every',type=int,default=300);ap.add_argument('--download-workers',type=int,default=16)
    ...
    train=group_subset(build_records(args.evidence_dir,'train'),args.train_per_group,args.pos_cap,args.seed)
    val=group_subset(build_records(args.evidence_dir,'validation'),args.val_per_group,args.pos_cap,args.seed+1)
    train=cache_records(train,Path(args.cache_dir)/'train',args.download_workers);val=cache_records(val,Path(args.cache_dir)/'validation',args.download_workers)
    test=None
    if args.evaluate_test:
        test=build_records(args.evidence_dir,'test');test=cache_records(test,Path(args.cache_dir)/'test',args.download_workers)
    ...
    tr=AugmentedDataset(train,syn,dup,args.seed);tl=DataLoader(tr,batch_size=args.batch_size,shuffle=True,num_workers=0,collate_fn=collate_train,drop_last=True)
    vl=DataLoader(MagnetogramDataset(val),batch_size=args.batch_size,shuffle=False,num_workers=0,collate_fn=collate_eval)
    ...
    # LOCKED across arms: augmentation changes examples only, not the loss weighting.
    fixed_pos_weight=torch.tensor(base_neg/max(1,base_pos),device=device,dtype=torch.float32);crit=FocalBCE(fixed_pos_weight,args.gamma)
    ...
    vp=predict(model,vl,device);thr,vm=threshold(vp);vp.to_csv(out/'validation_predictions.csv',index=False)
    added=(len(pd.read_csv(syn)) if syn else dup)
    report={'arm':args.arm,'seed':args.seed,'locked_test':not args.evaluate_test,'architecture':{'width':args.width,'dropout':args.dropout,'loss':'focal','gamma':args.gamma,'lr':args.lr},'parameters':parameter_count(model),'fixed_steps':args.steps,'fixed_real_only_pos_weight':float(fixed_pos_weight.item()),'real_train_rows':len(train),'real_positive_rows':base_pos,'added_positive_rows':added,'total_train_items':len(tr),'validation_threshold':thr,'validation':vm,'history':history}
    if test is not None:
