...
def soft_pil_gradient_score(
    ...
    return (contact*grad).sum((1,2,3))/(contact.sum((1,2,3))+1e-6)
...
def pil_distribution_loss(fake_b: torch.Tensor, real_b: torch.Tensor, pixel_mm: float = 2.0) -> torch.Tensor:
    # Log-compress the scalar statistic so the loss is dimensionless and is not
    # numerically dominated by a few extreme G/Mm values.
    f=torch.log1p(soft_pil_gradient_score(fake_b,pixel_mm)/50.0).unsqueeze(1)
    r=torch.log1p(soft_pil_gradient_score(real_b.detach(),pixel_mm)/50.0).unsqueeze(1)
