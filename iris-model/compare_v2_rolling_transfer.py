...
PRIMARY='tss'
DEFAULT_ARMS=['real','duplicate','base','hj','pil','hj_pil','pil_blur','geometry_flip','block_shuffle']
...
def main():
    ...
    comparisons=[]
    for arm in args.arms:
        if arm in ('real','duplicate'): continue
        if all(arm in d for d in folds.values()): comparisons.append((arm,'duplicate'))
    for arm in ('duplicate','base','hj','pil','hj_pil'):
