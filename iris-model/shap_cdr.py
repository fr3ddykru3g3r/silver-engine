...
class ShapOutputWrapper(torch.nn.Module):
    """Present the binary logit as a one-column output for SHAP.

    FeatureTransformer intentionally returns shape (B,), while SHAP's PyTorch
    GradientExplainer indexes model outputs as (B, n_outputs). This wrapper changes
    only the output shape to (B,1); it does not alter logits or gradients.
    """
...
def main():
    ...
    ck=torch.load(a.checkpoint,map_location='cpu');seq_len=int(ck.get('seq_len',x.shape[1]));m=FeatureTransformer(nf,seq_len=seq_len);m.load_state_dict(ck['state_dict']);m.eval();wrapped=ShapOutputWrapper(m).eval()
    ...
    if sv.shape[:2]!=(len(x),x.shape[1]) or sv.shape[-1]!=nf:
    ...
    rep={'model':a.model,'partition':a.partition,'samples':len(x),'background_samples':len(bg),'sequence_length':int(x.shape[1]),'features':names,'checkpoint':str(a.checkpoint),'note':'global importance is mean absolute SHAP summed over the 40 temporal positions; local CSV retains signed temporal sums. Output wrapper changes shape only, not model values.'}
