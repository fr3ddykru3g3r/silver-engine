...
`report.pdf` is the compiled 70-page report for the completed IRIS BASE run and its 100-step HJ/PIL screening follow-up. It covers the verified FITS acquisition, chronology and label audits, the local step-400 → step-1200 resume, 128 generated arrays, independent generic/geometry/PIL audits, the corrected metric negative control, an alternate sampler seed, selective-destruction controls, the v2 physics self-test, three cached physics screens, 2D/3D figures, OBJ visualization meshes, limitations, and the exact future gates that remain open.
