...
The correct next sequence is therefore: restore the verified inputs; run a converged BASE train-only audit; fine-tune the HJ and HJ+PIL arms from the same BASE checkpoint; select or reject coefficients using only train/validation physics evidence; and only after that gate passes, run the exact primary matrix \texttt{R}/\texttt{Rw}/\texttt{D}/\texttt{L0}/\texttt{L2}/\texttt{L3}. The exploratory morphology summary is retained as a negative control for protocol discipline, not counted as completion of that sequence.
...
The package is reproducible at the level of the captured source, evidence, cache, checkpoints, outputs, figures, tables, and hashes. The requested GitHub branch and release asset are now remotely verified: commit \texttt{25fe07971e607ddac8fef459cb6b886e2bc3f41d} is present on branch \texttt{codex/iris-base-report-2026-08-31}, and the full archive is present in release \texttt{iris-2026-08-31} with its recorded byte count and SHA-256. The raw FITS archive is too large for an ordinary Git commit, so it is preserved as the verified release asset while the report, manifests, checksums, and code are committed directly.
...
GitHub preservation & PASS & Branch commit and release asset verified remotely; full archive size is 2,125,488,303 bytes and SHA-256 is \texttt{5315c4184caf6e96c100b4d304c777e17a14d86625f3c8465ac32b88b13e985e}. \\
