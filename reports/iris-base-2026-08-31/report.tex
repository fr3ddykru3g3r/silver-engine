...
% Keep chapters in the narrative flow. The default report class clears a full
% page before every chapter, which created artificial lower-page whitespace in
% this evidence-heavy document.
\makeatletter
...
\input{tables/generated_values.tex}
...
\centering
\vspace*{12mm}
{\LARGE\bfseries IRIS BASE Experiment\par}
\vspace{7mm}
{\large A comprehensive evidence report on verified solar magnetogram acquisition,\\
conditional diffusion generation, distributional fidelity, and negative results\par}
\vspace{14mm}
{\large IRIS / Silver Engine research record\par}
\vspace{4mm}
{\large 31 August 2026\par}
\vspace{16mm}
\noindent\fcolorbox{red}{red!4}{\parbox{0.86\textwidth}{\centering\small\textbf{Interpretive status.} This report contains the original BASE audit, the 100-step exploratory HJ/PIL screen, and a completed remote 1,200-step train-only continuation. The fresh remote BASE, HJ, and HJ+PIL arms all failed the generic fidelity gate, so the downstream forecasting matrix was correctly locked. No forecasting utility, causal magnetic-structure, or physical-realism claim is supported.}}\par
\vspace{16mm}
{\small Complete report package: figures, tables, machine-readable metadata, source code,\\
OBJ visualization meshes, checkpoints, acquisition receipts, and the combined archive.\par}
\end{titlepage}

\chapter*{Executive summary}
\addcontentsline{toc}{chapter}{Executive summary}

This report documents the IRIS BASE experiment and its completed remote continuation as an auditable research object. The objective was narrower than proving a useful flare forecaster: first obtain a verified set of historical HMI/SHARP line-of-sight magnetograms, preserve a chronology-safe data split, train the declared BASE conditional diffusion generator, draw synthetic positive-conditioned samples, and evaluate them with an independent descriptor audit. A later compute-bounded 100-step positive-only HJ/PIL factorial screen was run from the frozen BASE EMA checkpoint, followed by a remote 1,200-step BASE/HJ/HJ+PIL train-only gate. That continuation completed its model and audit jobs but failed the generic fidelity requirement, so the exact downstream forecasting matrix remained locked. The final verified result is therefore a negative train-only gate result, not an end-to-end forecasting result.
...
The strongest conclusion is therefore negative and useful: the original BASE checkpoint cleared a lenient broad gate, but the fresh remote BASE continuation failed it at a generic distance ratio of 112.144. The remote HJ and HJ+PIL arms improved their targeted geometry and PIL distances by 71.22\%/98.65\% and 69.22\%/97.87\%, respectively, yet still failed their generic gates at ratios 9.417 and 10.502. The report records the completed additional analysis, the gate receipt, the downstream lock, all major limitations, and the exact next experiments required before any utility claim.
...
The report supports six bounded claims. First, the specified historical FITS acquisition completed with a machine-readable PASS receipt. Second, the declared local BASE resume completed at step 1,200 and produced checkpoints, samples, training history, and audit files. Third, the original BASE audit passed its broad gate under the corrected evaluator. Fourth, the audit exposes a substantial residual mismatch, so that BASE result is not sufficient to unlock a scientific conclusion about physics transfer. Fifth, the physics implementation self-test passed and the three 100-step exploratory arms executed with active intended losses, but all three failed the broad generic gate at that early budget. Sixth, the remote 1,200-step BASE/HJ/HJ+PIL continuation completed its train-only jobs, failed the generic gate, and correctly prevented downstream evaluation.

The report does not support a claim of improved TSS, AUROC, AUPRC, HSS2, calibration, or any other forecasting metric: the downstream matrix was skipped after the train-only gate failed. It does not support a claim that synthetic fields are physically possible coronal configurations. It does not support a claim that the model learned causal flare precursors. It does not support a claim that a single seed or a single checkpoint is converged. The 100-step physics screen is an early training diagnostic; the remote 1,200-step continuation is a completed negative gate, not a forecast result.

The later GitHub morphology-transfer summary was separately downloaded and reconciled. It is useful exploratory evidence, but its arms are \texttt{base}, \texttt{pil}, \texttt{pil\_blur}, \texttt{geometry\_flip}, and \texttt{block\_shuffle}; it is not the locked primary matrix. The full-source protocol defines the exact primary mapping as \texttt{R}=\texttt{real} unweighted, \texttt{Rw}=\texttt{real\_weighted} with balanced positive weighting, \texttt{D}=\texttt{duplicate}, \texttt{L0}=\texttt{base}, \texttt{L2}=\texttt{hj}, and \texttt{L3}=\texttt{hj\_pil}. A dependency-free validator is included to reject the exploratory artifact and to enforce shared test identities and matched positive exposure before primary metrics are accepted.

\section{What was treated as an instruction versus a request}

The user’s request was to complete and document the run, produce a reusable package, and pursue the next analyses. The attached project documents specify the intended data rules, independent audit definitions, and stop conditions. Their role was to constrain the analysis, not to manufacture missing outcomes. For example, the project’s readiness document says that a high forecast score alone would never satisfy the gate; that statement is compatible with this report’s emphasis on data integrity and manipulation checks. The short physics screen below exercises the implementation, and the remote continuation records the completed 1,200-step train-only gate. The documents’ full physics manipulation, rolling replication, human audit, and prospective blind-validation requirements remain open because the generic gate failed before utility evaluation.

The continuation review also identified a nomenclature mismatch between an older seven-arm implementation and the requested primary matrix. The older morphology controls remain auxiliary. The validator and the frozen protocol file in the package make the distinction explicit: \texttt{Rw} is not a new data source, but a real-only control whose positive-class weight is computed once from the real training subset; \texttt{L0}, \texttt{L2}, and \texttt{L3} are the three synthetic additions that enter the primary comparison. This prevents a morphology-bootstrap result from being relabeled as a physics-transfer result.
...
The original BASE arm adds the generic calibration proxy with coefficient $\lambda_g=0.08$. The physics coefficients were retained in its checkpoint metadata for protocol consistency but its BASE contributions are zero because that condition is BASE; the separate remote HJ and HJ+PIL arms are reported in the continuation section. The training history records the combined loss, denoising component, generic component, and the cosine learning rate.
...
\IfFileExists{tables/seed_robustness.tex}{\input{tables/seed_robustness.tex}}{\textit{The alternate-seed table is generated after the seed-2027 audit.}}
...
The report’s data audit, independent feature statistics, 3D exports, ensemble spread, metric correction, alternate sampling seed, v2 physics self-test, 100-step HJ/PIL screen, and remote 1,200-step train-only continuation were all executed as part of the paper build. They are not speculative recommendations. Their scripts and machine-readable outputs are included in the package. The remote gate failed before utility evaluation, so the full physics-to-utility result remains open.
...
\IfFileExists{tables/physics_screening.tex}{\begin{table}[p]\centering\resizebox{\textwidth}{!}{\input{tables/physics_screening.tex}}\caption{Screening-arm metrics. The table is exploratory and is not a downstream utility result.}\label{tab:physics-screening}\end{table}}{\textit{Physics-screening table not generated.}}

\section{Remote 1,200-step train-only continuation}

To complete the next declared gate without heating the workstation, the optimized workflow was moved to GitHub Actions. Run \texttt{33469232832} restored the frozen evidence, reused the validated BASE FITS cache, trained a fresh positive-only BASE arm for 1,200 steps, initialized L2/HJ and L3/HJ+PIL from its identical checkpoint, and ran the independent train-only audits. All three model jobs completed successfully. The machine-readable receipt is \texttt{artifacts/remote\_physics\_gate\_33469232832.json}; the run and artifact inventory are recorded in \texttt{artifacts/remote\_primary\_run\_33469232832.md}.

The result is a strict negative gate. The fresh remote BASE arm failed the generic fidelity gate at ratio 112.144, with hard-geometry distance 2.0618 and hard-PIL distance 52.0751. L2/HJ reduced those targeted distances to 0.5934 and 0.7041, corresponding to 71.22\% and 98.65\% improvement, but its generic ratio remained 9.417 and its generic gate failed. L3/HJ+PIL reduced them to 0.6347 and 1.1102, corresponding to 69.22\% and 97.87\% improvement, but its generic ratio remained 10.502 and its generic gate also failed.

\begin{table}[htbp]
\centering\small
\caption{Completed remote train-only gate. Targeted distances improved, but the broad generic gate failed for every fresh arm.}
\label{tab:remote-physics-gate}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lrrrrrr}
\toprule
Arm & Generic ratio & Geometry distance & PIL distance & Geometry improvement & PIL improvement & Gate \\
\midrule
BASE & 112.1441 & 2.0618 & 52.0751 & -- & -- & FAIL \\
L2/HJ & 9.4172 & 0.5934 & 0.7041 & 71.22\% & 98.65\% & FAIL \\
L3/HJ+PIL & 10.5017 & 0.6347 & 1.1102 & 69.22\% & 97.87\% & FAIL \\
\bottomrule
\end{tabular}}
\end{table}

Because the generic gate failed, the workflow did not train or evaluate the exact primary forecasting matrix \texttt{R}/\texttt{Rw}/\texttt{D}/\texttt{L0}/\texttt{L2}/\texttt{L3}. No forecast metrics were accessed. This is not a stalled run: it is the protocol’s intended fail-closed outcome, and it identifies the next work as improving or replicating the generator under a frozen train-only admission test before any utility claim.
...
The local BASE cache contains \AcquiredCount{} FITS payloads, while the eligible primary manifest contains \PrimaryCount{} rows. A separate remote acquisition run, 33470782803, then validated the complete downstream plan: 10,486/10,486 required FITS records, with zero missing and zero invalid, in the retained artifact \texttt{iris-complete-downstream-fits} (3,972,017,708 bytes). That complete cache was not silently substituted into the failed physics gate; the physics run’s reproducibility boundary remains the declared 5,273-file BASE seed. The full cache should not be confused with the selected training subset used here. A manifest-level acquisition receipt should record scope, missing counts, invalid counts, and checksums for every stage.
...
The input labels and chronological splits are designed for forecasting, but no forecaster was trained in this execution. The remote train-only physics gate failed, so the generated arrays were not inserted into an evaluation fold and no TSS or other forecast utility metric was produced. It would be scientifically incorrect to infer a forecasting result from the generator audit or from the targeted-distance improvements. Any future report must retain this boundary in its abstract, figures, and README.
...
The project’s 90-percent readiness document still lists full physics manipulation, rolling replication, prospective freeze, human audits, and archival eligibility as incomplete. The v2 self-test, 100-step screening arms, and remote 1,200-step BASE/HJ/HJ+PIL train-only continuation are now executed evidence, but the remote continuation failed the generic gate and therefore did not authorize downstream evaluation. These are not “missing pages” in this report; they are explicitly bounded scientific gates. The report is comprehensive about the runs that occurred, not a substitute for the remaining confirmatory experiments.

\section{Continuation gate and artifact reconciliation}

The full FITS cache, evidence manifest, and captured BASE checkpoint are present in the continuation archive and are checked by \texttt{scripts/validate\_primary\_matrix.py}. The validator is deliberately fail-closed. It refuses an artifact that contains only morphology arms, refuses an incomplete six-arm set, rejects unequal test identities, rejects unequal added-positive counts for \texttt{D}/\texttt{L0}/\texttt{L2}/\texttt{L3}, and rejects any explicit use of test metrics for selection. It is a structural admission check, not a replacement for the independent physics evaluator.

The correct next sequence is therefore: preserve the negative gate; improve or replicate the generator under a predeclared train-only protocol; select or reject coefficients using only train/validation physics evidence; and only after a fresh gate passes, run the exact primary matrix \texttt{R}/\texttt{Rw}/\texttt{D}/\texttt{L0}/\texttt{L2}/\texttt{L3}. The exploratory morphology summary is retained as a negative control for protocol discipline, not counted as completion of that sequence.

\chapter{Conclusion}

The IRIS BASE run has been acquired, verified, executed, sampled, audited, visualized, and packaged. The original data path passed for \AcquiredCount{} FITS records. The local model reached step \BaseSteps{} from a step-400 resume and generated \SyntheticCount{} positive-conditioned arrays. The original broad generic gate passed with flux, area, diversity, and saturation checks inside their declared floors. A fresh remote 1,200-step BASE/HJ/HJ+PIL continuation then completed its train-only audits but failed the generic admission gate.

The more important result is the limitation: the original corrected generic distance is \AuditDistanceRatio{} times the real split-half p90 reference, while the fresh remote BASE ratio was 112.144. The original six-dimensional metric was unstable because of near-zero saturation dispersion, and the remote constrained arms still failed the generic gate despite large targeted-distance improvements. The correct scientific conclusion is that BASE is an operational baseline with substantial residual distribution mismatch. It should be improved or replicated under the frozen train-only gate, not used as evidence of forecast improvement or a finished physical simulator.

The package is reproducible at the level of the captured source, evidence, BASE cache, checkpoints, outputs, figures, tables, and hashes. The remote physics artifacts and complete downstream FITS artifact are retained with their workflow runs; the exact final commit, archive byte count, and SHA-256 are recorded in the external \texttt{IRIS\_ARCHIVE\_AUDIT\_2026-09-01.json} receipt and checked by the fail-closed preservation verifier. The raw FITS archive is too large for an ordinary Git commit, so it is preserved through the release/action artifacts while the report, manifests, checksums, and code are committed directly.
...
\IfFileExists{tables/partition_summary.tex}{\input{tables/partition_summary.tex}}{\textit{Generated table missing.}}

\section{Acquired-cache table}
\IfFileExists{tables/acquired_cache_summary.tex}{\input{tables/acquired_cache_summary.tex}}{\textit{Generated table missing.}}

\section{Generic descriptor table}
\IfFileExists{tables/generic_descriptor_statistics.tex}{\input{tables/generic_descriptor_statistics.tex}}{\textit{Generated table missing.}}

\section{Hard descriptor table}
\IfFileExists{tables/hard_descriptor_statistics.tex}{\input{tables/hard_descriptor_statistics.tex}}{\textit{Generated table missing.}}

\section{Independent line-of-sight proxy tables}
\IfFileExists{tables/physics_proxy_statistics.tex}{\input{tables/physics_proxy_statistics.tex}}{\textit{Generated physics-proxy table missing.}}
\IfFileExists{tables/physics_proxy_bootstrap.tex}{\input{tables/physics_proxy_bootstrap.tex}}{\textit{Generated physics-proxy bootstrap table missing.}}

\section{Two-sample classifier audit}
\IfFileExists{tables/two_sample_classifier_summary.tex}{\input{tables/two_sample_classifier_summary.tex}}{\textit{Generated classifier table missing.}}

\section{Conditional proxy audit}
\IfFileExists{tables/conditional_proxy_summary.tex}{\input{tables/conditional_proxy_summary.tex}}{\textit{Generated conditional-proxy table missing.}}

\section{Gate stability audit}
\IfFileExists{tables/gate_stability_summary.tex}{\input{tables/gate_stability_summary.tex}}{\textit{Generated gate-stability table missing.}}

\section{Memorization and nearest-neighbor audit}
\IfFileExists{tables/memorization_distance_summary.tex}{\input{tables/memorization_distance_summary.tex}}{\textit{Generated memorization table missing.}}

\section{Training summary}
\IfFileExists{tables/training_summary.tex}{\input{tables/training_summary.tex}}{\textit{Generated table missing.}}

\section{Alternate seed and destruction controls}
\IfFileExists{tables/seed_robustness.tex}{\input{tables/seed_robustness.tex}}{\textit{Generated seed table missing.}}
\IfFileExists{tables/destruction_controls.tex}{\input{tables/destruction_controls.tex}}{\textit{Generated destruction-control table missing.}}

\section{Physics screening}
\IfFileExists{tables/physics_screening.tex}{\input{tables/physics_screening.tex}}{\textit{Generated physics-screening table missing.}}
\IfFileExists{tables/bootstrap_descriptor_uncertainty.tex}{\input{tables/bootstrap_descriptor_uncertainty.tex}}{\textit{Generated bootstrap table missing.}}
...
\midrule
\endhead
40+ page report & PASS & Compiled PDF is 61 dense A4 pages after removing forced blank figure pages. \\
...
Future-direction experiments & PARTIAL & Self-test passed, three 100-step screens ran, and remote 1,200-step BASE/HJ/HJ+PIL arms completed; the generic gate failed before downstream utility. \\
Downstream forecasting & DEFERRED & Physics gate required first; forecast not run. \\
GitHub preservation & PASS & Branch commit and release asset are verified remotely; exact commit, byte count, and SHA-256 are recorded in the external archive-audit receipt. \\
Local deletion & NOT STARTED & Remote preservation is verified; deletion remains withheld pending an explicit, bounded IRIS-only cleanup action. \\
\bottomrule
\end{longtable}

\section{Safe cleanup boundary}

The raw cache and temporary run roots are valuable for reproduction and were intentionally retained while the report was being built. The workspace also contains an unrelated La Liga project, so a command such as ``delete all files'' would be unsafe and outside a precise IRIS-only boundary. Remote preservation has now succeeded. If cleanup is later explicitly confirmed, the safe candidate set is only the named IRIS temporary roots and IRIS report/archive copies; unrelated project files must not be removed.

The combined archive is 2,125,488,303 bytes (about 1.98 GiB, or 2.13 GB in decimal units) because it contains the raw FITS cache. Ordinary GitHub repositories reject individual files above the standard large-file limit, so direct Git publication contains the report, code, manifests, checksums, and a documented pointer to the large archive; the raw archive is preserved through the verified GitHub Release asset rather than silently split into opaque chunks.
