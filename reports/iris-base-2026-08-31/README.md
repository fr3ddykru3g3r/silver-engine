...
`report.pdf` is the compiled 53-page report for the completed IRIS BASE run, its 100-step HJ/PIL screening follow-up, and the completed remote 1,200-step train-only continuation. It covers the verified FITS acquisition, chronology and label audits, the local step-400 → step-1200 resume, 128 generated arrays, independent generic/geometry/PIL audits, the corrected metric negative control, an alternate sampler seed, selective-destruction controls, the v2 physics self-test, three cached physics screens, the remote BASE/HJ/HJ+PIL gate, cluster-bootstrap uncertainty intervals, an independent thirteen-feature line-of-sight magnetic/spectral/topology proxy audit, a group-held-out real-versus-generated classifier audit, a latitude-conditioned proxy audit, connected-group gate-stability bootstrap analysis, exact-hash and nearest-neighbor memorization checks, the mathematical HJ/PIL proxy definitions, five offline protocol self-tests, 2D/3D figures, OBJ visualization meshes, the reconciled exploratory GitHub artifact, the exact primary R/Rw/D/L0/L2/L3 protocol, limitations, and the future gates that remain open. Figures now flow with the narrative; forced one-figure pages were removed rather than counted as report content.

The primary scientific result is deliberately bounded: the original BASE broad calibration gate passes for seed 2026, but the corrected generic distance is 7.681 times the real split-half p90 reference and the joint gate passes in only 0.508 of connected-group bootstrap replicates. A second sampler seed fails that gate at 11.055. The 100-step HJ, PIL, and HJ+PIL screens activate their intended losses but fail the broad generic gate at 20.855, 21.521, and 20.081 times the real split-half p90 reference. In the completed remote continuation, fresh BASE, HJ, and HJ+PIL generic ratios were 112.144, 9.417, and 10.502; targeted geometry/PIL distances improved, but the generic gate failed and locked the six-arm forecast matrix. No forecasting result or causal physics claim is supported.
...
`artifacts/reconciliation_note_2026-09-01.md`, `V2_DOWNSTREAM_MATRIX_FREEZE_2026-08-27.md`, and `scripts/validate_primary_matrix.py` reconcile the exploratory morphology artifact with the exact primary matrix. The validator defines `Rw` as the balanced-weight real-only control and refuses to promote morphology-only arms into the primary result.

The `remote/` directory contains the cache-backed physics workflow and the separate complete-FITS acquisition workflow. `scripts/acquire_complete_fits.py` seeds the 5,273-file BASE cache, fetches only the missing records required by the exact downstream sampling plan, and writes a fail-closed cache receipt before the files are used. The complete remote acquisition passed in Actions run `33470782803` with 10,486/10,486 required records; its artifact is retained remotely. The 1,200-step physics continuation is recorded in `artifacts/remote_primary_run_33469232832.md`; it failed the train-only generic gate, so the six-arm forecast matrix was correctly skipped.
